...
import styles from './ProductByBrand.module.scss';
...
function ProductByBrand() {
    ...
    useEffect(() => {
        const fetchEntity = async () => {
            try {
                const response = await getBrandBySlug(id);
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            ...
            try {
                const response = await getProducts({ searchBy: 'brandSlug', keyword: id, ...filters });
    ...
    return (
        <div className="container">
            <Breadcrumb
                ...
                items={[{ title: 'Trang chủ', to: '/' }, { title: brand.name }]}
            />
            ...
            <div className="bg-white p-4 rounded shadow-sm">
                <div className="row">
                    <div className="col-12">
                        <ReactQuill value={brand.description} readOnly={true} theme="bubble" />
                    </div>
                </div>
            </div>
        </div>
